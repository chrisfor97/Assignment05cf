...
venues_de_df<-data.frame()
for (p in 1:de_length){
  elements<-as.numeric(length(venueslist_de2[[p]]))
  ...
  venues_de_df2<-data.frame(unlist(names),unlist(postal_Code),unlist(city),unlist(longitude),unlist(latitude),unlist(url),unlist(address))
  venues_de_df<-rbind(venues_de_df,venues_de_df2)
}
venues_de_df<-venues_de_df %>%rename(name=unlist.names.,
         city=unlist.city.,
...
venues_de_df$latitude<-venues_de_df$latitude  %>% as.numeric()
venues_de_df$longitude<-venues_de_df$longitude  %>% as.numeric()
head(venues_de_df)
```
